...
from pathfinding.astarpath import find_astar_path
import json
...
def info():
    return {
        ...
        "color": "#9933ff",  # TODO: Choose color
...
def move(game_state: typing.Dict) -> typing.Dict:
    ...
    for snake in opponents:
        ...
        if snake["name"] != my_snake_name:
            ...
            # add the tails of every other snakes
            opponent_tails.append(Location(snake["body"][len(snake["body"]) - 1]["x"],snake["body"][len(snake["body"]) - 1]["y"]))
    ...
    food_path = find_astar_path(my_snake, food_destination, obstacles)
    ...
    if (food_path != None and len(food_path) > 1):
        ...
        future_tail_path = find_astar_path(future_snake, future_snake[len(future_snake)-1], obstacles)
    ...
    if next_move == "unknown":
        # if we cannot chase our tail, then we will chase other snake's tail
        for tail_destination in opponent_tails:
            tail_finder = AStarFinder(my_snake, tail_destination, obstacles, board_width, board_height, 2)
            tail_path = tail_finder.findthepath()
            if (tail_path != None and len(tail_path) > 1):
                print("Found opponent tail path: ", tail_path)
                next_move = get_next_move(tail_path[0], tail_path[1])
                break
